...
mod imp {
    ...
    impl WidgetImpl for HealthGraphView {
            ...
            */
            ...
            for i in 0..5 {
                ...
                let layout = widget.create_pango_layout(Some(
                    &((((inner.height - HALF_Y_PADDING * 2.0) / inner.scale_y / 4.0)
                        * (4 - i) as f32) as u32)
                        .to_string(),
                ));
            ...
            */
            ...
            for (i, point) in inner.points.iter().enumerate() {
                ...
                cr.move_to(
                    ...
                    (inner.height + HALF_Y_PADDING) as f64
        ...
        fn size_allocate(&self, _widget: &Self::Type, width: i32, height: i32, _baseline: i32) {
            ...
            if inner.points.is_empty() {
                ...
                // using even numbers
                biggest_value = biggest_value % X_LINES_INTERVAL;

                inner.scale_x = (inner.width - HALF_X_PADDING * 2.0) / inner.points.len() as f32;
                inner.scale_y = (inner.height - HALF_Y_PADDING * 2.0) / biggest_value;
            }
        }
    }
    ...
    impl ObjectImpl for HealthGraphView {
        fn constructed(&self, obj: &Self::Type) {
            ...
            gesture_controller.connect_pressed(clone!(@weak obj => move |c, _, x, y| imp::HealthGraphView::from_instance(&obj).on_motion_event(x, y, true, c)));
            obj.add_controller(&gesture_controller);

            let motion_controller = gtk::EventControllerMotion::new();
            motion_controller.connect_enter(clone!(@weak obj => move|c, x, y| imp::HealthGraphView::from_instance(&obj).on_motion_event(x, y, false, c)));
            motion_controller.connect_motion(clone!(@weak obj => move|c, x, y| imp::HealthGraphView::from_instance(&obj).on_motion_event(x, y, false, c)));
    ...
    impl HealthGraphView {
        ...
        pub fn set_points(&self, obj: &crate::views::HealthGraphView, points: Vec<Point>) {
            if points.len() > 30 {
                obj.set_size_request(50 * 30, -1);
            } else {
                obj.set_size_request((50 * points.len()).try_into().unwrap(), -1);
            }

            self.inner.borrow_mut().points = points;
