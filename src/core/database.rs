...
use uom::si::{length::meter, u32::Length};
...
impl HealthDatabase {
    ...
    pub async fn get_activities(&self, date_opt: Option<DateTime<FixedOffset>>) -> Result<Vec<Activity>, glib::Error> {
        ...
        while cursor.next_async_future().await.is_ok() {
            ...
            for i in 0..cursor.get_n_columns() {
                match cursor.get_variable_name(i).unwrap().as_str() {
                    "id" => activity.activity_type = ActivityType::from_i64(cursor.get_integer(i)).unwrap(),
                    "date" => activity.date = DateTime::parse_from_rfc3339(cursor.get_string(i).0.as_str()).unwrap(),
                    "calories_burned" => activity.calories_burned = Some(cursor.get_integer(i) as u32),
                    "distance" => activity.distance = Some(Length::new::<meter>(cursor.get_integer(i) as u32)),
                    "heart_rate_avg" => activity.heart_rate_avg = Some(cursor.get_integer(i) as u32),
                    "heart_rate_max" => activity.heart_rate_max = Some(cursor.get_integer(i) as u32),
                    "heart_rate_min" => activity.heart_rate_min = Some(cursor.get_integer(i) as u32),
    ...
    pub async fn get_steps(&self, date: DateTime<FixedOffset>) -> Result<Vec<Steps>, glib::Error> {
        ...
        while cursor.next_async_future().await.is_ok() {
            ...
            hashmap.insert(date, hashmap.get(&date).unwrap_or(&0) + cursor.get_integer(1) as u32);
        }

        Ok(hashmap.drain().map(|(date, steps)| {
            Steps::new(date, steps)
        }).collect())
    }

    pub async fn get_weights(&self, date: DateTime<FixedOffset>) -> Result<Vec<Weight>, glib::Error> {
        ...
        while cursor.next_async_future().await.is_ok() {
            ret.push (Weight::new(DateTime::parse_from_rfc3339(cursor.get_string(0).0.as_str()).unwrap(), uom::si::f32::Mass::new::<uom::si::mass::kilogram>(cursor.get_double (1) as f32)));
        }
    ...
    pub async fn reset(&self) -> Result<(), glib::Error> {
        self.connection.update_async_future("DELETE WHERE { ?datapoint a health:WeightMeasurement }").await?;
        self.connection.update_async_future("DELETE WHERE { ?datapoint a health:Activity }").await?;
    ...
    pub async fn save_activity(&self, activity: Activity) -> Result<(), glib::Error> {
        ...
        resource.set_int64("health:activity_id", activity.activity_type.to_u32().unwrap().into());

        if let Some(c) = activity.calories_burned {
            resource.set_int64("health:calories_burned", c.into());
        }
        if let Some(d) = activity.distance {
            resource.set_int64 ("health:distance", d.get::<uom::si::length::kilometer>().into());
        }
        ...
        self.connection.update_async_future(resource.print_sparql_update(Some(&self.manager), None).unwrap().as_str()).await?;
        Ok(())
    }
    ...
    pub async fn save_weight(&self, weight: Weight) -> Result<(), glib::Error> {
        ...
        resource.set_double ("health::weight", weight.weight.get::<uom::si::mass::kilogram>().into());

        self.connection.update_async_future(&format!("DELETE WHERE {{ ?u health:weight_date {} }}; {}", &weight.date.to_rfc3339(), resource.print_sparql_update(Some(&self.manager), None).unwrap().as_str())).await?;
        Ok(())
    }
}
